...
def update_customer_info():
    ...
    req_mobile= request.args.get('mobile')
    result = session.query(ProductEnquiry).filter(ProductEnquiry.mobileNumber == req_mobile , ProductEnquiry.sentToDealer == False)\
                                          .update({'sentToDealer' : True})
    session.commit()
    set_customer_status(req_mobile)
