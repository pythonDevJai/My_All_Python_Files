...
@app.route('/update-record-studentinfo' , methods = ['PATCH'])
def update_record():
    request_name = request.args.get('name')
    request_mobile = int(request.args.get('mobile'))
    update_result = session.query(StudentInfo).filter(StudentInfo.Name == request_name).update({"mobile_number" : request_mobile })
    session.commit()
